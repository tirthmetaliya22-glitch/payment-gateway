...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    socket.on('payment_update', (data: any) => {
      ...
      // Check if this update is for the current payment ID and type is PAYMENT_PAID
      if (data.type === 'PAYMENT_PAID' && data.order_id === params.id) {
        console.log('[Checkout] Payment paid event received for current order! Updating status to Paid.');

        // Update local state to show the success screen
  ...
  useEffect(() => {
    if (isCashfreeLoaded && (window as any).Cashfree && paymentData) {
      try {
        ...
        console.log("Payment Data:", upiQr);
        ...
        <div className="w-full max-w-[340px] sm:max-w-[380px] bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 p-8 text-center space-y-6 animate-in fade-in zoom-in duration-500">
          ...
          <div className="bg-slate-50 rounded-2xl p-4 space-y-3 border border-slate-100">
            ...
            <div className="flex justify-between text-[10px] font-black px-2">
              ...
              <span className="text-secondary tracking-[0.2em]">{paymentData.utr_id}</span>
            </div>
            <div className="flex justify-between text-[10px] font-black px-2 items-center">
              ...
              <span className="text-green-600 uppercase tracking-widest bg-green-100/50 px-3 py-1 rounded-full border border-green-200/50">Processing</span>
            </div>
          </div>
          <div className="pt-4">
            <button
  ...
  const amount = paymentData.amount.replace('Γé╣', '');
      ...
      <div className="w-full max-w-[340px] sm:max-w-[380px] bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100 animate-in fade-in zoom-in duration-300">
        ...
        <div className="p-5 space-y-5">
          <div className="text-center py-2">
            ...
            <h2 className="text-2xl font-bold text-secondary tracking-tight">Γé╣{amount}</h2>
          </div>
